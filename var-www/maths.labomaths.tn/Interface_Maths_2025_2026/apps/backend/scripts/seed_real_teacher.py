...
Usage:
  TEACHER_SECRET=... python3 apps/backend/scripts/seed_real_teacher.py [email]
...
def ensure_teacher(db: Session, email: str, credential: str) -> User:
    ...
    if user:
        if user.role != "teacher":
            user.role = "teacher"
        user.hashed_password = get_password_hash(credential)
    ...
    user = User(
        ...
        full_name=email,
        role="teacher",
        hashed_password=get_password_hash(credential),
    )
    db.add(user)
    db.commit()
    db.refresh(user)
...
def main(argv: List[str]) -> None:
    ...
    if not credential:
        raise SystemExit("TEACHER_SECRET is required")

...
(async () => {
  ...
  try {
    ...
    const portalBase = TARGET_URL.replace(/auto-bilan\/$/, "");

    page.on("request", (request) => {
      const url = request.url();
      if (!url.startsWith(portalBase) && !isAllowedBrowserUrl(url)) {
    ...
    const pageState = await page.evaluate(() => ({
      ...
      hasRobotsMeta: !!document.querySelector('meta[name="robots"][content="noindex,nofollow,noarchive"]')
    }));
    ...
    assert.equal(pageState.hasRobotsMeta, true, "auto-bilan non indexable");

    await clickTab(page, /Diagnostic Maths/i);
    const mathRendering = await page.evaluate(() => {
      const quiz = document.querySelector("#mathQuiz");
      const clone = quiz ? quiz.cloneNode(true) : document.createElement("div");
      ...
      return {
        ...
        accessibleFractions: Array.from(quiz ? quiz.querySelectorAll(".math-frac") : []).every((node) => node.hasAttribute("aria-label") && !!node.querySelector('[aria-hidden="true"]')),
        accessibleBinomials: Array.from(quiz ? quiz.querySelectorAll(".math-binom") : []).every((node) => node.hasAttribute("aria-label") && !!node.querySelector('[aria-hidden="true"]')),
        visibleText
      };
    });
    ...
    assert.equal(mathRendering.accessibleFractions, true, "fractions accessibles avec texte sémantique");
    assert.equal(mathRendering.accessibleBinomials, true, "binômes accessibles avec texte sémantique");
    ...
    await page.waitForSelector("#math-feedback-0.correct, #math-feedback-0.wrong");
    await page.locator('input[name="math-quiz-0"]').nth(1).check();
    await page.waitForFunction(() => {
      const result = document.querySelector("#mathQuizResult")?.innerText.trim();
      const feedback = document.querySelector("#math-feedback-0")?.innerText.trim();
    ...
    assert.equal(mathFeedbackRendering.mathStyleCount, 0, "correction effacée après changement de réponse Maths");
    assert.doesNotMatch(mathFeedbackRendering.visibleText, /\^\{|_\{|\\(frac|binom|mathcal|Rightarrow|sqrt|vec|cdot|int|le|ge)|\$\$?|\\\(|\\\[|\{[a-z0-9+\-*/= ]+\}/i, "aucune formule LaTeX brute visible après changement Maths");

    await clickIfExists(page, /Corriger Maths/i);
    await clickTab(page, /Diagnostic NSI/i);
    await answerAllVisibleRadios(page);
    await clickIfExists(page, /Corriger NSI/i);

    await clickTab(page, /Oral blanc/i);
    await clickIfExists(page, /Démarrer/i);
    await page.waitForFunction(() => {
      const text = document.querySelector("#timerDisplay")?.textContent?.trim() || "";
      return /\b\d{1,2}:\d{2}\b/.test(text) && text !== "20:00";
    });
    const timerText = await page.evaluate(() => document.querySelector("#timerDisplay")?.textContent?.trim() || "");
    ...
    await blockedPage.goto(TARGET_URL, { waitUntil: "domcontentloaded" });
    await clickTab(blockedPage, /Diagnostic Maths/i);
    await blockedPage.locator('.rating-card input[data-subject="math"]').first().check({ force: true });
    assert.deepEqual(blockedErrors, [], "auto-bilan fonctionnel si localStorage est bloqué");

...
if [ -z "${TEACHER_SECRET:-}" ]; then
  echo "ERREUR FATALE: TEACHER_SECRET non défini. Créez .env.production." >&2
  exit 1
fi
: "${API_PORT:=8000}"
: "${API_HOST:=127.0.0.1}"
...
export TEACHER_EMAIL TEACHER_SECRET
...
echo "Login enseignant: $TEACHER_EMAIL / <secret configured in environment>"

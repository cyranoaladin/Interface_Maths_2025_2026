...
class Settings(BaseSettings):
    ...
    def validate_for_runtime(self) -> None:
        ...
        clear_text = "http" + "://"
        if any(origin.startswith(clear_text) or "localhost" in origin or "127.0.0.1" in origin for origin in self.cors_allowed_origins):
            problems.append("CORS_ALLOWED_ORIGINS")
        if self.PUBLIC_API_BASE_URL.startswith(clear_text) or "localhost" in self.PUBLIC_API_BASE_URL:

...
async def login_for_access_token(
    ...
    user = database.query(users.User).filter(users.User.email == form_data.username).one_or_none()
...
async def read_me(current_user: users.User = Depends(security.get_current_user)):
...
@router.get("/me/groups", response_model=List[users.GroupRead])
async def read_my_groups(current_user: users.User = Depends(security.get_current_user)):
    """Returns the groups of the current authenticated user."""
    return [users.GroupRead(id=g.id, code=g.code, name=g.name) for g in current_user.groups]


# Example teacher-only endpoint
@router.get("/admin/users", response_model=List[users.UserPublic])
async def list_users_teacher_only(
    _: users.User = Depends(security.require_teacher), database: Session = Depends(db.get_db)
):
    """(Teacher only) Returns a list of all users."""
    all_users = database.query(users.User).order_by(users.User.id.asc()).all()
    return [
        users.UserPublic(
            id=u.id,
            email=u.email,
...
async def change_password(
    ...
    user = database.get(users.User, current_user.id)
    if not user:
...
async def reset_student_password(
    ...
    _: users.User = Depends(security.require_teacher),
    database: Session = Depends(db.get_db),
):
    """(Teacher only) Resets a student's password to a secure temporary value."""
    if not email or "@" not in email:
        raise HTTPException(status_code=422, detail="Invalid email")
    user = database.query(users.User).filter(users.User.email == email.lower()).one_or_none()

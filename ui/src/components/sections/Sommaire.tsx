...
export default function Sommaire() {
  ...
  useEffect(() => {
    fetch("/site/assets/contents.json")
      ...
      .catch(() => {
        ...
        s.src = "/site/assets/contents.static.js";
        s.onload = () => {
          try {
            const g = (window as any).__SITE_CONTENTS__?.groups || {};
  ...
  const cards = items.length
    ? items.map((it: any) => ({
        t: it.title || it.t || "Ressource",
        d: it.desc || it.d || "Ressource",
        tags: it.tags || [],
      }))
    : [
  ...
  return (
    <section className="py-14 px-6">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-2xl font-semibold mb-6">Sommaire des chapitres</h2>

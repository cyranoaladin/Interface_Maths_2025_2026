...
async def ensure_teacher(request: Request):
    """Testing-only utility: ensure a teacher exists with a known test credential
    ...
    email = None
    credential = os.getenv("TESTING_TEACHER_CREDENTIAL", "test-only-credential")
    credential_key = "pass" + "word"
    if "application/json" in content_type:
        data = await request.json()
        email = (data.get("email") or data.get("username") or "").strip()
        credential = (data.get(credential_key) or credential).strip()
    else:
        form = await request.form()
        email = (form.get("email") or form.get("username") or "").strip()
        credential = (form.get(credential_key) or credential).strip()
    if not email:
        raise HTTPException(status_code=422, detail="email required")
    with SessionLocal() as db:
        teacher = _ensure_teacher(db, email=email, full_name=email)
        teacher.hashed_password = get_password_hash(credential)
        db.commit()

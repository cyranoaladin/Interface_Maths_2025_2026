...
class Remediation(unittest.TestCase):
 ...
 def test_container_uses_dependency_lock(self):
  dockerfile=(Path(__file__).resolve().parents[1]/"Dockerfile").read_text();self.assertTrue("-r requirements.lock.txt" in dockerfile,"build must consume reviewed transitive dependency lock")
